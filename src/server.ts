...
server.tool("create-random-user", "Create a random user with fake data ", {
    ...
    const res = await server.server.request(
        {
            ...
            params: {
                messages: [{
                    ...
                    content: {
                        type: "text",
                        text: "Generate a random user with fake data. The user should have a realistic name, email, address and phone number. Return this data as a JSON object. with other text or formatter so it can be used with JSON.parse."
                    }
                }],
                maxTokens: 1024
            }
    ...
    try {
        const fakeUser = JSON.parse(res.content.text.trim().replace(/^\s*```json\s*/, "").replace(/```$/, "").trim());
        const id = await createUser(fakeUser)
        return {
            content: [
                {
                    type: "text",
                    text: `User ${id} created successfully with fake data`
                }
            ]
        }
